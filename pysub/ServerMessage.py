...
class GameConfigMessage(ServerMessage):
    ...
    def __init__(self, message, socket_manager):
        ...
        for _ in range(self.settings_count):
            self.custom_settings.append(socket_manager.receive_message(self))
...
class ActivationMessage(TurnMessage):
    """Base message for equipment activations in the turn"""
    min_part_count = 3
    def __init__(self, prefix, message_type, message):
        super().__init__(self, prefix, message_type, self.min_part_count, message)

class SonarDetectionMessage(ActivationMessage):
    """Message recording sonar activations in the turn"""
    prefix = "S"
    message_type = "sonar detection"

    def __init__(self, message):
        super().__init__(self.prefix, self.message_type, message)
...
class DiscoveredObjectMethod(TurnMessage):
...
class GameFinishedMessage(ServerMessage):
    ...
    def __init__(self, message):
        ...
        for _ in range(self.player_count):
            self.player_results.append("TODO") #receive message
